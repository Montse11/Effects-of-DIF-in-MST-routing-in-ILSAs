...
#acountry = loadObject("D:/DIF in MST Routing in ILSA/DIF in MST Analysis/Results Latent Regression/Person Parameter Results/Country and mstpath/NoDIF.1_p3.Rbin")
...
latent_reg = function(data, j, k, l, m){
  ...
  #Save both item parameters and the object
   ...
   saveObject(estimated_items, file = paste0(path5,"NoDIF.",  k, "_p", j,".Rbin"))
   path6 <- "D:/DIF in MST Routing in ILSA/DIF in MST Analysis/Results Latent Regression/Item Parameters/"
   saveObject(tam.mod, file = paste0(path6,"NoDIF.", k, "_p", j,".Rbin"))
  ...
  #saveObject(latent_model, file = paste0(path2,"NoDIF.",  k, "_p", j,".Rbin"))
  ...
  saveObject(Results, file = paste0(path4, "Country and mstpath/" ,"NoDIF.",  k, "_p", j,".Rbin"))
 # saveObject(Results_bycountry, file = paste0(path4, "Country/" ,"NoDIF.",  k, "_p", j,".Rbin"))
...
itembank = read.csv("D:/DIF in MST Routing in ILSA/Simulation MST/Data/item bank for modules.csv", 
                    header = T)
Slope = itembank[, 4]
Location = itempar[, 1] # this is for selecting the NODIF item parameters
it = data.frame(a = Slope, b = Location)

# Call simulated results
setwd("D:/DIF in MST Routing in ILSA/DIF in MST Results/NoDIF/Reference/Reference.p1/")
...
clusterExport(cl,list('latent_reg', 'NoDIF', 'it', 'background_10', 'R.utils',
                      'saveObject'), envir=environment())

#latent_reg(NoDIF, j=3, k = 1, l = "NoDIF", m = "NoDIF" )

parSapply(cl, 1:100, function(j) latent_reg(NoDIF, j, k = 1, l = "NoDIF", m = "NoDIF" ))
...
#======
# Call simulated results
setwd("D:/DIF in MST Routing in ILSA/DIF in MST Results/NoDIF/Reference/Reference.p2/")
...
parSapply(cl, 1:100, function(j) latent_reg(NoDIF, j, k = .7, l = "NoDIF", m = "NoDIF" ))
...
#=====
# Call simulated results
setwd("D:/DIF in MST Routing in ILSA/DIF in MST Results/NoDIF/Reference/Reference.p3/")
...
parSapply(cl, 1:100, function(j) latent_reg(NoDIF, j, k = .5, l = "NoDIF", m = "NoDIF" ))
