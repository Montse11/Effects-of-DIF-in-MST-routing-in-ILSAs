...
#install.packages("C:/Users/monts/Box Sync/DIF in MST/Simulation MST/mstRforLR/v1.1/mstRforLR_1.1.zip", repos = NULL, type = "win.binary")
install.packages("D:/DIF in MST Routing in ILSA/Simulation MST/mstRforLR/v1.2/mstRforLR_1.2.zip", repos = NULL, type = "win.binary")
library("mstRforLR") 
setwd("D:\\DIF in MST Routing in ILSA\\Simulation MST\\mstR first change")
...
#################################################################################
#Set pathway 
path <- "D:\\DIF in MST Routing in ILSA\\Simulation MST\\Data"
setwd(path)


# Call true item parameters
itempar = read.csv("Test assemble/itempar.no.dif.csv", header = T)
itembank = read.csv("item bank for modules.csv", header = T)
Slope = itembank[, 4]
Location = itempar[, 2] # this is for selecting the DIF introduced to only 11%
...
test1 <- list(moduleSelect="MFI", method = "EAP",  prob=c(1,1))
...
path <- "G:/DIF in MST Routing in ILSA/New Results/"
Ntheta = 1000
NoReps <- c(3000)
nrSamples = 100
...
for (i in 1:100){ 
  ...
  ressim <- NULL
  for (k in 1:NoReps[1]){
    test2 <- list(method = "EAP", moduleSelect= "MFI", prob=c(1,1), seed.prob = (i*k+47405))
  ...
  saveObject(data, file = paste0(path, "Focal_NoDIF_p1.1_p2.1", "_i", i, ".Rbin"))
...
for (i in 51:100){ 
  ...
  ressim <- NULL
  for (k in 1:NoReps[1]){
    test2 <- list(method = "EAP", moduleSelect= "MFI", prob=c(.7,.7), seed.prob = (i*k+47405))
  ...
  saveObject(data, file = paste0(path, "NoDIF.p2", "_i", i, ".Rbin"))
...
for (i in 51:100){ 
  ...
  saveObject(data, file = paste0(path, "NoDIF.p3", "_i", i, ".Rbin"))
...
################################################################################
#Install and call package (this version was specifically created for Leslie Rutkowski et al.)
install.packages("C:/Users/mbvaldiv/Desktop/Simulation MST/mstRforLR/v1.1/mstRforLR_1.1.zip", repos = NULL, type = "win.binary")
library("mstRforLR") 
install.packages("R.utils")
library("R.utils")
install.packages("TAM")
...
#################################################################################
#Set pathway 
path <- "C:/Users/mbvaldiv/Desktop/Simulation MST/Data"
setwd(path)
...
#################################################################################
#                         NO DIF MST SIMULATION  Reference                              #
#################################################################################
...
data <- expand.grid(rep = NA,
                    Group = "Reference",
                    Country = c(rep("SGP", 1000), rep("KOR", 1000), rep("TWN", 1000),
                                rep("SWE", 1000), rep("MLT", 1000), rep("MYS", 1000)), 
                    DIFmag = "NoDIF", 
                    DIFper = "NoDIF",
                    DIFloc = "NoDIF", 
                    Prob = "1", 
...
for (i in 51:100){ 
  set.seed(i*1992)
  th <- theta[[i]] <-  c(rnorm(Ntheta, mean= 1.21, sd= .82),
                         rnorm(Ntheta, mean= 1.06, sd= .85),
                         rnorm(Ntheta, mean= 0.99, sd= .97),
                         rnorm(Ntheta, mean= 0.01, sd= .72),
                         rnorm(Ntheta, mean= -0.06, sd= .88),
                         rnorm(Ntheta, mean= -0.35, sd= .87)) 
  ressim <- NULL
  for (k in 1:NoReps[1]){
  ...
  saveObject(data, file = paste0(path, "NoDIF.R.p1", "_i", i, ".Rbin"))
...
for (i in 51:100){ 
  ...
  saveObject(data, file = paste0(path, "NoDIF.R.p2", "_i", i, ".Rbin"))
...
for (i in 51:100){ 
  ...
  saveObject(data, file = paste0(path, "NoDIF.R.p3", "_i", i, ".Rbin"))
